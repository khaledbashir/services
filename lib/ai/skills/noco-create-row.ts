...
const skill: Skill = {
  ...
  description: 'Create a new row in a NocoDB table. Pass the table_id (from noco_list_tables) and a fields object whose keys match the table\'s column titles. Returns the created row including its Id.',
  ...
  async handler(args) {
    ...
    const created = await Noco.createRow(String(args.table_id), fields)
    return { created }
  },
}
