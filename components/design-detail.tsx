...
/**
 * The full ticket detail body, minus any page chrome. Rendered two ways:
 *  - as the standalone /designs/[id] page (wrapped in DashboardLayout below), and
 *  - embedded in the Design board's right-hand panel (Charlie 2026-07-15) so a
 *    designer can click through their queue without leaving the board.
 * `embedded` swaps the back button for a Close action and lets card clicks on
 * the board swap the panel instead of navigating.
 */
export function DesignDetailBody({
  ...
  const updateField = async (payload: Record<string, any>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        try { window.dispatchEvent(new Event('anc:data-refresh')) } catch {}
      }
    } catch (err) {
      console.error(err)
    } finally {
  ...
  const duplicate = async () => {
    ...
    try {
      ...
      if (newId) {
        if (embedded && onNavigate) onNavigate(newId)
        else router.push(`/designs/${newId}`)
      }
    } catch (err) {
      console.error(err)
  ...
  const setInternalCategory = async (next: string | null) => {
    try {
      const res = await fetch(`/api/design-requests/${id}/internal-category`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const setPriority = async (next: 'high' | null) => {
    try {
      const res = await fetch(`/api/design-requests/${id}/priority`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const saveAsTemplate = async () => {
    ...
    try {
      const res = await fetch(`/api/design-requests/${id}/save-as-template`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (loading) {
    return (
      <div className={embedded ? 'space-y-6' : 'max-w-[1800px] mx-auto space-y-6 py-2'}>
  ...
  if (!dr) {
    return (
      <div className={embedded ? 'py-20 text-center text-sm text-zinc-400' : 'max-w-5xl mx-auto py-20 text-center text-sm text-zinc-400'}>
  ...
  return (
    <div className={embedded ? 'space-y-6' : 'max-w-5xl mx-auto space-y-6 py-2'}>
        {/* Header */}
