...
export function Sidebar() {
  ...
  const DESIGNER_ALLOWED_HREFS = new Set([
    ...
    // Request Hub is the company-wide intake — every role can submit/follow.
    '/request-hub', '/request-hub/new',
  ])
  const DESIGN_CONSULTANT_ALLOWED_HREFS = new Set(['/designs', '/cg-designs', '/request-hub', '/request-hub/new'])

  const sectionVisibleForUser = (section: NavSection): boolean => {
    if (isDesignContractor) {
      return section.key === 'design' || section.key === 'request-hub'
    }
    if (isDesigner) {
      return section.key === 'events' || section.key === 'design' || section.key === 'request-hub'
    }
  ...
  const sections: NavSection[] = useMemo(() => [
    ...
    {
      // One front door for ideas / builds / changes / problems (2026-07-29).
      // Everyone can submit and follow; leadership gets the decision table.
      key: 'request-hub',
      label: 'Request Hub',
      icon: <Icon><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 13.5h3.86a2.25 2.25 0 012.012 1.244l.256.512a2.25 2.25 0 002.013 1.244h3.218a2.25 2.25 0 002.013-1.244l.256-.512a2.25 2.25 0 012.013-1.244h3.859m-19.5.338V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18v-4.162c0-.224-.034-.447-.1-.661L19.24 5.338a2.25 2.25 0 00-2.15-1.588H6.911a2.25 2.25 0 00-2.15 1.588L2.35 13.177a2.25 2.25 0 00-.1.661z" /></Icon>,
      role: 'technician',
      links: [
        { href: '/request-hub', label: 'Requests' },
        { href: '/request-hub/new', label: 'New Request' },
        { href: '/admin/request-hub', label: 'Configuration', role: 'admin' },
      ],
    },
    {
      key: 'design',
    ...
    },
    {
      ...
      links: [
        ...
        { href: '/venue-intelligence', label: 'Venue Intelligence' },
