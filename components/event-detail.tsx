...
/**
 * Full event detail body. Rendered two ways:
 *  - the standalone /events/[id] page (wrapped in DashboardLayout), and
 *  - embedded in the Events / My Assignments split panel (Charlie 7/27) so
 *    scheduling never needs a page hop.
 */
export function EventDetailBody({
  ...
  const eventId = id
  const cameFromVenue = !embedded && searchParams?.get('from') === 'venue'
      ...
      <div className="space-y-8">
        ...
        <div>
          <div className="flex items-center justify-between gap-3 mb-4">
            {embedded ? (
              <button onClick={() => onClose?.()} className="text-sm text-[#0A52EF] hover:text-[#0840C0] font-medium">
                ← Close
              </button>
            ) : (
              <Link
...
function EventFrame({ embedded, children }: { embedded: boolean; children: React.ReactNode }) {
