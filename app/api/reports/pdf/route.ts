...
async function getReportData(period: string, venueId?: string) {
  ...
  const [totalR, coveredR, workflowR, laborR, marketR, topStaffR, topVenuesR, ticketsR, prevTotalR, prevCoveredR, prevWfR, upcomingUnassignedR] = await Promise.all([
    query(`SELECT COUNT(*) as total FROM events e WHERE COALESCE(e.approval_status, 'approved') = 'approved' AND e.event_date >= $1 AND e.event_date <= $2 ${vf}`, params),
    query(`SELECT COUNT(DISTINCT e.id) as covered FROM events e JOIN event_assignments ea ON e.id = ea.event_id WHERE COALESCE(e.approval_status, 'approved') = 'approved' AND e.event_date >= $1 AND e.event_date <= $2 ${vf}`, params),
    query(`SELECT COUNT(*) as total, COUNT(CASE WHEN events.workflow_status = 'post_game_submitted' THEN 1 END) as completed FROM events WHERE COALESCE(approval_status, 'approved') = 'approved' AND event_date >= $1 AND event_date <= $2 ${vfEvents} AND EXISTS (SELECT 1 FROM event_assignments ea WHERE ea.event_id = events.id)`, params),
    query(`SELECT COALESCE(SUM(ea.estimated_hours), 0) as total_hours, COUNT(DISTINCT ea.staff_id) as unique_staff FROM event_assignments ea JOIN events e ON ea.event_id = e.id WHERE COALESCE(e.approval_status, 'approved') = 'approved' AND e.event_date >= $1 AND e.event_date <= $2 ${vf}`, params),
    query(`SELECT m.name as market, COUNT(e.id) as events, COUNT(DISTINCT CASE WHEN ea.event_id IS NOT NULL THEN e.id END) as covered, COALESCE(SUM(ea.estimated_hours), 0) as hours FROM events e JOIN venues v ON e.venue_id = v.id JOIN markets m ON v.market_id = m.id LEFT JOIN event_assignments ea ON e.id = ea.event_id WHERE COALESCE(e.approval_status, 'approved') = 'approved' AND e.event_date >= $1 AND e.event_date <= $2 ${vf} GROUP BY m.name ORDER BY events DESC`, params),
    query(`SELECT s.full_name, COUNT(ea.id) as events, COALESCE(SUM(ea.estimated_hours), 0) as hours, COUNT(CASE WHEN e.workflow_status = 'post_game_submitted' THEN 1 END) as completed FROM event_assignments ea JOIN staff s ON ea.staff_id = s.id JOIN events e ON ea.event_id = e.id WHERE COALESCE(e.approval_status, 'approved') = 'approved' AND e.event_date >= $1 AND e.event_date <= $2 ${vf} GROUP BY s.id, s.full_name ORDER BY hours DESC LIMIT 5`, params),
    ...
    query(`SELECT COUNT(*) as total, COUNT(CASE WHEN status = 'closed' THEN 1 END) as resolved, COUNT(CASE WHEN priority = 'critical' THEN 1 END) as critical, COUNT(CASE WHEN sla_response_met = true THEN 1 END) as sla_met, COUNT(CASE WHEN sla_response_met = false THEN 1 END) as sla_breached, AVG(CASE WHEN resolved_at IS NOT NULL THEN EXTRACT(EPOCH FROM (resolved_at - created_at))/3600 END) as avg_resolution FROM tickets WHERE created_at >= $1 AND created_at <= $2::date + 1 ${vfTickets}`, params),
    // Previous period for trends
    query(`SELECT COUNT(*) as total FROM events e WHERE COALESCE(e.approval_status, 'approved') = 'approved' AND e.event_date >= $1 AND e.event_date < $2 ${vf}`, prevParams),
    query(`SELECT COUNT(DISTINCT e.id) as covered FROM events e JOIN event_assignments ea ON e.id = ea.event_id WHERE COALESCE(e.approval_status, 'approved') = 'approved' AND e.event_date >= $1 AND e.event_date < $2 ${vf}`, prevParams),
    query(`SELECT COUNT(*) as total, COUNT(CASE WHEN events.workflow_status = 'post_game_submitted' THEN 1 END) as completed FROM events WHERE COALESCE(approval_status, 'approved') = 'approved' AND event_date >= $1 AND event_date < $2 ${vfEvents} AND EXISTS (SELECT 1 FROM event_assignments ea WHERE ea.event_id = events.id)`, prevParams),
    // Upcoming unassigned (next 7 days)
    query(`SELECT COUNT(*) as count FROM events e JOIN venues v ON e.venue_id = v.id WHERE COALESCE(e.approval_status, 'approved') = 'approved' AND e.event_date > $1 AND e.event_date <= $1::date + 7 AND v.requires_assignment = true ${vfEnd} AND NOT EXISTS (SELECT 1 FROM event_assignments ea WHERE ea.event_id = e.id)`, endParams),
  ])
